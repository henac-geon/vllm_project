...
# 웹 페이지나 API에서 데이터를 가져올 때 사용
import requests

# urllib.parse: URL 인코딩/디코딩을 위한 표준 라이브러리
# 검색 쿼리를 URL에 안전하게 포함시키기 위해 사용
...
# 한국 주식 시장 정보를 검색하는 함수 (AI가 호출할 수 있는 도구)
# @function_tool 데코레이터: 일반 Python 함수를 Agent가 호출 가능한 도구로 등록
@function_tool
def search_korean_stock(query: str) -> str:
    """한국 주식 시장 정보를 구글에서 검색합니다. 코스피, 코스닥, 특정 날짜의 주식 시장 동향 등을 검색할 때 사용합니다."""
    # 함수 호출 로그 출력 (디버깅용)
    print("=" * 60)
    print(f"[FUNCTION CALL] search_korean_stock() 함수가 호출되었습니다!")
    print(f"[SEARCH QUERY] {query}")
    print("=" * 60)

    try:
        # 구글 검색 URL 생성
        # 한국 주식 시장 관련 키워드를 추가하여 검색 정확도 향상
        search_query = f"한국 주식 시장 {query} 코스피 코스닥"
        encoded_query = quote(search_query)
        google_url = f"https://www.google.com/search?q={encoded_query}&hl=ko"

        # User-Agent 헤더 설정 (봇으로 인식되지 않도록)
        headers = {
            'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36'
        }

        print(f"[WEB REQUEST] 구글 검색 요청 중... URL: {google_url[:100]}...")

        # HTTP GET 요청 전송 (타임아웃 10초)
        response = requests.get(google_url, headers=headers, timeout=10)
        response.raise_for_status()

        print(f"[WEB RESPONSE] 응답 수신 완료 (Status: {response.status_code})")

        # 간단한 텍스트 파싱 (실제 프로덕션에서는 BeautifulSoup 사용 권장)
        # 여기서는 응답 텍스트에서 관련 정보를 추출
        text = response.text
        ...
        ※ 구글 검색을 통해 다음과 같은 정보를 찾았습니다:

        - 현재 실시간 정보를 확인하려면 다음 사이트를 방문하세요:
          • 네이버 증권: https://finance.naver.com
          • 한국거래소: https://www.krx.co.kr
          • 삼성증권: https://www.samsungpop.com

        - 코스피(KOSPI): 대형주 중심의 한국 종합주가지수
        - 코스닥(KOSDAQ): 중소형 및 벤처기업 중심의 시장

        ※ 참고: 실시간 주식 정보는 증권사 앱이나 금융 포털에서 확인하는 것을 권장합니다.
        최신 시장 동향은 전문 금융 정보 사이트에서 확인하세요.
        """

        print(f"[FUNCTION RESULT] 검색 결과 생성 완료")
        print("=" * 60)

        return result.strip()

    except requests.Timeout:
        error_msg = "검색 요청 시간 초과: 구글 서버 응답이 지연되고 있습니다."
        print(f"[ERROR] {error_msg}")
        print("=" * 60)
        return f"[오류] {error_msg}\n대신 네이버 증권(finance.naver.com)이나 한국거래소(krx.co.kr)를 방문해보세요."

    except requests.RequestException as e:
        error_msg = f"검색 요청 실패: {str(e)}"
        print(f"[ERROR] {error_msg}")
        print("=" * 60)
        return f"[오류] {error_msg}\n직접 검색 사이트를 방문하시거나 나중에 다시 시도해보세요."
...
# 예약을 처리하는 함수 (AI가 호출할 수 있는 도구)
# @function_tool 데코레이터: 일반 Python 함수를 Agent가 호출 가능한 도구로 등록
@function_tool
def make_reservation(location: str, time: str, people: int) -> str:
    """예약을 생성합니다. 레스토랑, 회의실, 공간 등의 예약 시 장소, 시간, 인원을 입력받아 예약을 처리합니다."""
    # 함수 호출 로그 출력 (디버깅용)
    print("=" * 60)
    print(f"[FUNCTION CALL] make_reservation() 함수가 호출되었습니다!")
    ...
    if not location or not time:
        error_msg = "예약 실패: 장소와 시간은 필수 항목입니다."
        print(f"[ERROR] {error_msg}")
        print("=" * 60)
        return f"[오류] {error_msg}"

    if people <= 0:
        error_msg = "예약 실패: 인원은 1명 이상이어야 합니다."
        print(f"[ERROR] {error_msg}")
        print("=" * 60)
        return f"[오류] {error_msg}"
    ...
    예약 번호: RES-{datetime.now().strftime('%Y%m%d%H%M%S')}

    ※ 예약 시간 10분 전까지 도착해 주시기 바랍니다.
    ※ 예약 변경이나 취소는 1시간 전까지 가능합니다.
    """

    # 성공 로그 출력
    print(f"[RESERVATION SUCCESS] {location}에 {people}명 예약 완료 ({time})")
    print("=" * 60)
...
def create_agent(system_prompt: str = "당신은 유능한 AI 어시스턴트입니다.") -> Agent:
    ...
    agent = Agent(
        ...
        # search_korean_stock: 한국 주식 시장 정보 검색에 자동으로 대응
        # make_reservation: 예약 요청에 자동으로 대응 (장소, 시간, 인원)
        tools=[get_current_time, get_user_info, search_korean_stock, make_reservation],
    )

    print(f"[AGENT CREATE] ✅ Agent 생성 완료 - tools: [get_current_time, get_user_info, search_korean_stock, make_reservation]")
    return agent

...
for pdf_file in pdf_files:
    ...
    print(f"   추출된 텍스트 : {cleaned_text} 문자")
# 전체 텍스트 합치기
...
def split_into_chunks(text, chunk_size=1000, overlap=100):
   ...
   def split_into_chunks_with_overlap(text, chunk_size=1000, overlap=100):
    chunks = []
    paragraphs = text.split('\n\n')
...
```
Without Overlap:          With Overlap (100자):
┌────────┐┌────────┐      ┌────────┐
